...
const CartContainer: FC<CartContainerProps> = ({disableInteractions = false}) => {
  ...
  return (
    ...
    <CartCard sx={{margin:{xs:'12px',md:'0'}, paddingX: '12px', paddingY: '14px'}}>
      ...
      {
        (isSuccess && cartCache && cartCache.cart.cartEntries.length !== 0) ? <CartEntryList cartEntries={cartCache.cart.cartEntries} disableInteractions={disableInteractions}/> : <div>No items in cart</div>
      }
    </CartCard>
  );
};
